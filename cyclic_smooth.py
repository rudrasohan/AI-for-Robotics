...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
	...
	while(change>=tolerance):
		...
		for i in range(len(path)):
			...
			newpath[i][0] += weight_smooth*(newpath[((i+1)%len(path))][0]+newpath[((i-1)%len(path))][0]-2*newpath[i][0])
			newpath[i][1] += weight_smooth*(newpath[((i+1)%len(path))][1]+newpath[((i-1)%len(path))][1]-2*newpath[i][1])
...
solution_check(smooth(testpath1), answer1)
printpaths(testpath1,smooth(testpath1))
solution_check(smooth(testpath2), answer2)
printpaths(testpath2,smooth(testpath2))
